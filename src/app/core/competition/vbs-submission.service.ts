  ...
  public reset(config: Config) {
    ...
    if (this._log) {
      ...
      this._interactionlogSubscription = DresTypeConverter.mapEventStream(this._eventbus.observable()).pipe(
          ...
          mergeMap((submission: QueryEventLog) => {
            ...
            return this._dresLog.postApiV2LogQueryByEvaluationId(this._dresService.activeRun().id, this._sessionId, submission).pipe(
                tap(o => {
    ...
    this._submitSubscription = this._submitSubject.pipe(
        ...
        mergeMap(([segment, frame, segmentId]) => {
          ...
          const submission = {} as ApiClientSubmission
          ...
          mergeMap((text) => {
            ...
            const submission = {
              answerSets: [{
                answers: [{
                  text: text
                } as ApiClientAnswer] as ApiClientAnswer[]
              } as ApiClientAnswerSet] as ApiClientAnswerSet[]
            }
